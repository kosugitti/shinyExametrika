...
#'     DO NOT REMOVE.
#' @param tabs Optional character vector of tab values to include (subset of the
#'     full set, e.g. c("tab_guide", "tab_data", "tab_ctt")). When `NULL`
#'     (default) all tabs are shown -- this is what the full shinyapps.io app
#'     uses. The shinylive per-function builds pass a subset. `tab_guide` and
#'     `tab_data` are always forced in so every build can load and format data.
#' @import shiny
#' @noRd
app_ui <- function(request, tabs = NULL) {
  ...
  tabs <- union(c("tab_guide", "tab_data"), tabs)  # always loadable + formattable
  has <- function(x) x %in% tabs

  # Build only the requested panels (lazily, so a build that omits a module's
  # source file never calls that module's *_ui()).
  panels <- list()
  if (has("tab_guide")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("Guide"), value = "tab_guide",
      mod_guide_ui("guide", i18n)
    )))
  }
  if (has("tab_data")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("Data"), value = "tab_data",
      mod_data_upload_ui("data_upload", i18n)
    )))
  }
  if (has("tab_descriptives")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("Descriptives"), value = "tab_descriptives",
      mod_descriptives_ui("descriptives", i18n)
    )))
  }
  if (has("tab_ctt")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("CTT"), value = "tab_ctt",
      mod_ctt_ui("ctt", i18n)
    )))
  }
  if (has("tab_irt")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("IRT"), value = "tab_irt",
      mod_irt_ui("irt", i18n)
    )))
  }
  if (has("tab_grm")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("GRM"), value = "tab_grm",
      mod_grm_ui("grm", i18n)
    )))
  }
  if (has("tab_lca")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("LCA"), value = "tab_lca",
      mod_lca_ui("lca", i18n)
    )))
  }
  if (has("tab_lra")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("LRA"), value = "tab_lra",
      mod_lra_ui("lra", i18n)
    )))
  }
  if (has("tab_biclustering")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("Biclustering"), value = "tab_biclustering",
      mod_biclustering_ui("biclustering", i18n)
    )))
  }
  if (has("tab_irm")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("IRM"), value = "tab_irm",
      mod_irm_ui("irm", i18n)
    )))
  }
  if (has("tab_bnm")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("BNM"), value = "tab_bnm",
      mod_bnm_ui("bnm", i18n)
    )))
  }
  if (has("tab_ldlra")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("LDLRA"), value = "tab_ldlra",
      mod_ldlra_ui("ldlra", i18n)
    )))
  }
  if (has("tab_ldb")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("LDB"), value = "tab_ldb",
      mod_placeholder_ui("ldb_placeholder", i18n)
    )))
  }
  if (has("tab_binet")) {
    panels <- c(panels, list(bslib::nav_panel(
      title = i18n$t("BINET"), value = "tab_binet",
  ...
  tagList(
    ...
    shinyjs::useShinyjs(),
    do.call(bslib::page_navbar, navbar_args)
  )
}
